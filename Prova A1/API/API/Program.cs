...
// DELETE: http://localhost:5273/tarefas/deletar/{id}
app.MapDelete("/tarefas/deletar/{id}", ([FromServices] AppDataContext ctx, string id) =>
{
    ...
    // Remover a tarefa do contexto e salvar as mudanças
    ctx.Tarefas.Remove(tarefa);
    ctx.SaveChanges();

    return Results.Ok($"Tarefa com ID {id} deletada com sucesso");
});
